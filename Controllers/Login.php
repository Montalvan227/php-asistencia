<?php 
	...
	class Login extends Controllers{
		...
		{
			$data['page_tag'] = "Login - Software Colegio";
			$data['page_title'] = "Software Colegio";
		...
		{
			$data['page_tag'] = "Recuperar Contraseña - Tienda Virtual";
			$data['page_title'] = "Tienda Virtual";
			$data['page_name'] = "login";
			$data['page_functions_js'] = "functions_login.js";
			$this->views->getView($this,"recuperar_contrasena",$data);
		}
			...
			if(empty($_POST['idUsuario']) || empty($_POST['txtEmail']) || empty($_POST['txtToken']) || empty($_POST['txtPassword']) || empty($_POST['txtPasswordConfirm'])){
					...
					if($strPassword != $strPasswordConfirm){
						...
						if(empty($arrResponseUser)){
							$arrResponse = array('status' => false, 
											 'msg' => 'Erro de datos.' );
						}else{$strPassword = hash("SHA256",$strPassword);
